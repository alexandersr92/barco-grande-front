import Image from "next/image";

// Banner "Descargá Avanz Móvil" con ola naranja y mockup del teléfono,
// se muestra sobre el footer en todo el sitio.
export default function AppDownloadBanner({
  appStoreUrl,
  playStoreUrl,
}: {
  appStoreUrl?: string;
  playStoreUrl?: string;
}) {
  return (
    <section className="relative overflow-hidden">
      <Image
        src="/icons/app-banner-wave.png"
        alt=""
        fill
        sizes="100vw"
        className="object-cover"
      />
      <div className="relative mx-auto flex min-h-[524px] max-w-[1220px] flex-col items-center justify-end gap-10 px-5 pt-40 lg:flex-row lg:items-end lg:gap-[158px] lg:pt-24">
        <div className="flex w-full max-w-[600px] flex-col gap-6 pb-[60px]">
          <h2 className="text-[34px] font-medium leading-[1.2] tracking-[-1px] text-white md:text-[44px]">
            Descargá Avanz Móvil, el banco en tus manos
          </h2>
          <p className="text-[17px] leading-7 text-white">
            Realiza transferencias y maneja tus cuentas las 24 horas del día
          </p>
          <div className="flex flex-wrap gap-4">
            <a
              href={appStoreUrl ?? "https://apps.apple.com"}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 rounded-[30px] bg-white px-9 py-[17px] text-base font-medium leading-[22.4px] text-secondary transition-colors hover:bg-secondary hover:text-white"
            >
              <svg width="20" height="24" viewBox="0 0 384 512" fill="currentColor" aria-hidden>
                <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z" />
              </svg>
              App Store
            </a>
            <a
              href={playStoreUrl ?? "https://play.google.com"}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 rounded-[30px] bg-white px-9 py-[17px] text-base font-medium leading-[22.4px] text-secondary transition-colors hover:bg-secondary hover:text-white"
            >
              <svg width="20" height="22" viewBox="0 0 512 512" fill="currentColor" aria-hidden>
                <path d="M325.3 234.3 104.6 13l280.8 161.2-60.1 60.1zM47 0C34 6.8 25.3 19.2 25.3 35.3v441.3c0 16.1 8.7 28.5 21.7 35.3l256.6-256L47 0zm425.2 225.6-58.9-34.1-65.7 64.5 65.7 64.5 60.1-34.1c18-14.3 18-46.5-1.2-60.8zM104.6 499l280.8-161.2-60.1-60.1L104.6 499z" />
              </svg>
              Google Play
            </a>
          </div>
        </div>
        <div className="relative hidden h-[500px] w-[450px] shrink-0 overflow-hidden lg:block">
          <Image
            src="/icons/app-phone.png"
            alt="App Avanz Móvil"
            width={741}
            height={713}
            className="absolute -left-[100px] top-[60px] w-[600px] max-w-none"
          />
        </div>
      </div>
    </section>
  );
}
